...
function getManufacturers(locale: string) {
  ...
  if (locale === 'en') return manufacturers_english;
...
export default function AddCarListingForm() {
  ...
  const uploadImages = async (files: File[]) => {
    ...
    return files.map((_, index) => `mock-image-${index + 1}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Car listing data:', carListingData);
      alert('Car listing submitted successfully! (Mock)');
      ...
      alert(t('error_message') || 'Error submitting form');
    } finally {
          ...
          >
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="yes" className="text-sm font-medium text-gray-700">{t('yes') || 'Yes'}</label>
              </div>
              <div className="flex items-center space-x-2">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <label htmlFor="no" className="text-sm font-medium text-gray-700">{t('no') || 'No'}</label>
              </div>
              <div className="flex items-center space-x-2">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <label htmlFor="maybe" className="text-sm font-medium text-gray-700">{t('maybe') || 'Maybe'}</label>
              </div>
            </div>
          </motion.div>
        );
          ...
          >
            ...
            <div className={`border-2 border-dashed rounded-xl p-8 transition-all duration-200 hover:border-blue-500 ${errors.images ? 'border-red-500' : 'border-gray-200'}`}>
              ...
              >
                ...
                <p className="text-lg font-medium text-gray-700 mb-2">{t('drag_drop') || 'Drag & Drop Images'}</p>
                <p className="text-sm text-gray-500">{t('image_requirements') || 'Upload up to 8 images'}</p>
              </label>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          ...
          {currentStep === STEPS.length - 1 ? (
            ...
            >
              {isSubmitting ? (t('submitting') || 'Submitting...') : (t('submit_listing') || 'Submit Listing')}
            </button>
          ) : (
            <button
              type="button"
