...
import { Car, Star, MapPin, Clock, Phone, ArrowRight, Sparkles, Droplets, Shield, Brush } from 'lucide-react';
import { Img } from './Img';
...
const services = [
  {
    icon: Sparkles,
    titleKey: "exterior_detailing",
    descriptionKey: "exterior_detailing_desc",
    color: "from-blue-600 to-blue"
  },
  {
    icon: Droplets,
    titleKey: "interior_cleaning",
    descriptionKey: "interior_cleaning_desc",
    color: "from-blue-600 to-blue-800"
  },
  {
    icon: Shield,
    titleKey: "paint_protection",
    descriptionKey: "paint_protection_desc",
    color: "from-blue-500 to-blue-900"
  },
  {
    icon: Brush,
    titleKey: "ceramic_coating",
    descriptionKey: "ceramic_coating_desc",
          ...
          >
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-xl backdrop-blur-sm hover:bg-white/10 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <span className="text-sm sm:text-base font-medium">{t('rating_with_reviews')}</span>
              </div>
              
              <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-xl backdrop-blur-sm hover:bg-white/10 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}>
                <MapPin className="w-4 h-4 sm:w-5 sm:h-5 text-blue-300" />
                <span className="text-sm sm:text-base">{t('central_location')}</span>
              </div>
              ...
              <div className={`flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-white/5 rounded-xl backdrop-blur-sm hover:bg-white/10 transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <span className="text-sm sm:text-base">0544603725</span>
              </div>
            </div>
          ...
          >
            <div className="aspect-video w-full rounded-xl sm:rounded-2xl overflow-hidden shadow-xl sm:shadow-2xl">
              <Image
                ...
                width={1000}
                height={1000}
                alt="AUTO SPA DETAILING Showroom"
            ...
            >
              <p className="text-white text-sm sm:text-lg font-medium">
                {t('special_offer')}: {t('free_interior_fragrance')}
              </p>
            </motion.div>
          </motion.div>
        </div>
